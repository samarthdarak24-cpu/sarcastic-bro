"""
Simple LangChain-style Chatbot Router
...
from ..services.simple_langchain_chatbot import simple_langchain_chatbot

router = APIRouter(prefix="/simple-langchain-chat", tags=["Simple LangChain Chatbot"])
...
async def create_chat_session(request: SessionRequest):
    ...
    try:
        session_id = await simple_langchain_chatbot.create_session(request.user_id)
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if not request.session_id:
            request.session_id = await simple_langchain_chatbot.create_session(request.user_id)
        
        # Get response
        result = await simple_langchain_chatbot.chat(
...
async def stream_chat(request: ChatRequest):
    ...
    try:
        ...
        async def generate_stream():
            async for chunk in simple_langchain_chatbot.stream_chat(
                message=request.message,
                session_id=request.session_id,
                user_context=request.user_context
            ):
...
async def get_chat_history(session_id: str):
    ...
    try:
        history = await simple_langchain_chatbot.get_chat_history(session_id)
...
async def clear_session(session_id: str):
    ...
    try:
        success = await simple_langchain_chatbot.clear_session(session_id)
        if success:
...
async def health_check():
    """Health check for the Simple LangChain chatbot"""
    try:
        health = await simple_langchain_chatbot.health_check()
...
async def get_knowledge_base_stats():
    ...
    try:
        health = await simple_langchain_chatbot.health_check()
        return {
            "total_documents": health.get("knowledge_base_entries", 0),
...
async def demo_endpoint():
    """Demo endpoint to test the chatbot"""
    try:
        # Create a demo session
        session_id = await simple_langchain_chatbot.create_session("demo-user")
        ...
        return {
            "demo": "Simple LangChain Chatbot Demo",
            "session_id": session_id,
            "question": "How do I improve my soil health?",
